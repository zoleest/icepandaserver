...
//Draws the characther changers  - The normal first then the minified
    function drawCharacterChanger(data){
        data.charactersJson.forEach(function(character){
            ...
            characterImg.onclick= function(){changeCharacter(character.slug)};
            ...
            characterImgMini.onclick= function(){changeCharacter(character.slug)};
            document.getElementById('character-changer-mini').appendChild(characterImgMini);
        });
    }

    //draw recent comments to sidebar and topbar
    function drawRecentComments(data){

        for(let locationCounter = 0; locationCounter < data.locations.length; locationCounter++) {
            let locationAnchor = document.createElement('a');
            locationAnchor.className = "link link-danger";
            locationAnchor.href = "/locations?id=" + data.details[locationCounter].slug;

            let locationTextNode = document.createElement('h5');
            locationTextNode.textContent = data.locations[locationCounter]._id;
            ...
            for(let locationCounter = 0; locationCounter < data.locations.length; locationCounter++){
                ...
                mainAnchor.textContent = data.locations[locationCounter]._id;
...
;
                ...
                redirectingLinkAnchor.href="/locations?id="+data.details[locationCounter].slug;
    ...
    //LOADING RECENT COMMENTS AND WATCH FOR UPDATE

    $.ajax({
        url: '/recent-comments',
        success: function(response){
            drawRecentComments(response);

            let newCommentChecker = setInterval(
                function(){

                    $.ajax({
                        url: '/recent-comments/last-comment',
                        success: function(subresponse){
                            if(subresponse.lastCommentDate > response.lastCommentDate){
                                ...
                                if(Notification.permission !== 'granted'){
                                    ...
                                    const notification = new Notification("Új hozzászólás érkezett!", {'body':'Nézz fel a Szerepjáték.net-re, mert új hozzászólás érkezett! :)','icon':'/images/favicon/mstile-150x150.png'});
                                }
