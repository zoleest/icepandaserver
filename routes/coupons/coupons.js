...
const MongoDBCollection = {
    ...
    "coupons": MongoClient.db(config.databaseName).collection(config.databaseName + "Coupons")
};
...
router.get('/', async function (req, res) {
    ...
    if (req.session.loggedIn !== undefined) {
        ...
        if (code !== undefined) {
            ...
            let couponData = await MongoDBCollection.coupons.findOne({
                "coupon_code": code,
                "coupon_used": 0
            }, {projection: {"coupon_type": 1}});
            ...
            if (couponData !== null) {
                ...
                switch (couponData.coupon_type) {
                    case 'level_up':
                        ...
                        MongoDBCollection.coupons.updateOne({"coupon_code": code}, {
                            $set: {
                                "coupon_used": 1,
                                "coupon_used_by": userName
                            }
                        });
